...
def get_debug_mode(default_debug: bool) -> bool:
    """提示用户是否启用调试模式，如果输入无效则使用默认值。

    Args:
        default_debug (bool): 默认是否启用调试模式

    Returns:
        bool: 是否启用调试模式
    """

    prompt = f"是否启用调试模式？(y/n，回车默认 {'y' if default_debug else 'n'})："

    while True:
        user_input = input(prompt).strip().lower()

        if not user_input:
            print(f"✅ 使用默认调试模式：{'开启' if default_debug else '关闭'}")
